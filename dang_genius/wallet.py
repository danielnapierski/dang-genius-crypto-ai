...
def coinbase_get_balances() -> dict:
    ...
    lst_accounts = []
    while has_next:
        response = coinbase_connect(
            url_path="/api/v3/brokerage/accounts", limit=50, cursor=cursor
        )
    ...
    df_accounts.columns = tmp_df_accounts.columns.estimates.tolist()
    av_index = len(df_accounts.columns)
    df_accounts["available_balance"] = df_accounts["available_balance.value"].astype(
        float
    )
...
def get_balances() -> dict:
    try:
        ...
        cursor.execute(
            """SELECT id, exchange, symbol, MAX(timestamp), available FROM wallet 
...
def check_swap_funding(
    ...
    return (
        balances[exchange_a].get(symbol_a) > amount_a and balances[exchange_b].get(symbol_b) > amount_b
    )
